...
function runCli(args, env, entry = CLI) {
...
test("bundled dist CLI answers image questions like the source CLI", async (t) => {
  const gateway = http.createServer((req, res) => {
    let raw = "";
    req.on("data", (chunk) => (raw += chunk));
    req.on("end", () => {
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({ choices: [{ message: { content: "Delegated answer." } }] }));
    });
  });
  await new Promise((resolveListen) => gateway.listen(0, "127.0.0.1", resolveListen));
  t.after(() => gateway.close());

  const home = mkdtempSync(join(tmpdir(), "at-vision-bin-"));
  const image = join(home, "shot.png");
  writeFileSync(image, PNG);
  writeFileSync(
    join(home, "config.jsonc"),
  ...
  assert.equal(result.code, 0, result.stderr);
  assert.equal(JSON.parse(result.stdout).answers[0].answer, "Delegated answer.");
});
