...
function guardEntry(matcher) {
  return { matcher, hooks: [{ type: "command", command: nodeCmd(GUARD_SCRIPT) }] };
}
...
function runClaude(opts) {
  ...
  if (wants(opts, "statusline")) {
    if (opts.uninstall) {
      ...
      const command = nodeCmd(path.join(REPO_ROOT, "statusline", "claude", "statusline.mjs"));
...
function runOpencode(opts) {
  ...
  const target = pathToFileURL(GUARD_PLUGIN).href;
  const contents =
    `// Generated by agent-tooling installer — do not edit.\n` +
    `// Re-exports the repo guard plugin so opencode discovers it here while the\n` +
    `// implementation and shared rules stay single-source in the repo.\n` +
